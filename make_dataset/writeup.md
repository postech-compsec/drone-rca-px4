# run_plan_files.py
```bash
python make_dataset/run_plan_files.py --command "./run_one_plan.sh {plan}"
```
### plan 경로 지정
```bash
python make_dataset/run_plan_files.py make_qgc_plan/plans --command "./run_one_plan.sh {plan}"
```
### 중간부터 재개
```bash
python make_dataset/run_plan_files.py --command "./run_one_plan.sh {plan}" --start-from plan_0005.plan
```
## 옵션 요약
